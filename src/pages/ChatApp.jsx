...
export default function ChatApp() {
  ...
  const loginGuest = async () => {
    ...
    setName(data.name); setGuestToken(data.guestToken);
  ...
  return (
    <div className="chat-container">
      <h2>尋夢園男歡女愛聊天室</h2>
      {!joined ? <button onClick={loginGuest} className="login-btn">訪客登入</button> :
      <div className="user-header"><strong>Hi, {name}</strong> <button onClick={leaveRoom}>離開</button></div>}
          ...
          <div className="chat-input">
            ...
            {(chatMode==="private"||chatMode==="publicTarget") && 
              <select value={target} onChange={e=>setTarget(e.target.value)}>
                ...
                {userList.filter(u=>u.name!==name).map(u=>(<option key={u.id} value={u.name}>{u.name}</option>))}
              </select>
            }
        ...
        <div className="user-list">
          ...
          {userList.map(u=>(
            <div key={u.id} className={`user-item ${u.name===target?"selected":""}`} onClick={()=>{setChatMode("private");setTarget(u.name);}}>
              {aiAvatars[u.name] && <img src={aiAvatars[u.name]} className="user-avatar"/>}{u.name} (Lv.{u.level||1})
            </div>
          ))}
        </div>
      </div>
